...
def cartesian2spherical(wrap_centre, site_pose):
    ...
    if r != 0:  # calculate theta and phi, if r != 0
        theta = np.arctan2(
            np.sqrt(np.sum((site_pose[0:2] - wrap_centre[0:2]) ** 2)), site_pose[2] - wrap_centre[2]
        )  # \theta = arctan2(y, x)
...
def TaitBryanRotationMatrix(Sequence, angles):
    ...
    if Sequence == "xzy":
        ...
        rot = [
            [cos(a) * cos(b), sin(a) * sin(r) - cos(a) * cos(r) * sin(b), cos(r) * sin(a) + c1 * s2 * s3],
            [s2, c2 * c3, -c2 * s3],
