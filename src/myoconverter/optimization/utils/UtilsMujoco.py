...
from myoconverter.optimization.utils.UtilsRotation import cylindarical2cartesian, quaternionRotaion, spherical2cartesian
...
def _get_actuator_moment_matrix(mjc_model, mjc_data):
    ...
    raise ValueError(
        f"Unsupported actuator_moment shape {actuator_moment.shape} for model with nu={mjc_model.nu}, nv={mjc_model.nv}"
    )
...
def getCoordinateRange_mjc(mjc_model):
    ...
    for ira, ra in enumerate(joint_ranges):
        if mjc_model.eq_obj1id is None or (
            not ra[0] == ra[1] and ira not in mjc_model.eq_obj1id[mjc_model.eq_type == 2]
        ):  # if there are no equality joint constraints
...
def calculateEndPoints_mjc(mjc_model_path, end_points, n_eval):
    ...
    for n in range(n_eval):
        ...
        for dof_id in free_jnt_id:
            ...
            jointEval.append(
                (
                    np.array(ang_range[0])
                    + 0.8 * np.random.random(1) * (np.array(ang_range[1]) - np.array(ang_range[0]))
                )[0]
            )
...
def sortMuscleWrapSiteJoint(mjc_model, muscle_para_osim):
    ...
    if tendon_name != f"{muscle_name}_tendon":
        raise RuntimeError(
            f"Could not find correct tendon. Found tendon is called {tendon_name}, "
    ...
    for site_id in range(site_num):  # run through these sites
        ...
        if mjc_model.wrap_type[i] == 4 or mjc_model.wrap_type[i] == 5:
            ...
            if wrap_id not in wrap_id_list:
            ...
            for i_jnts, joints in enumerate(muscle_para_osim["wrapping_coordinates"]):
                ...
                ma_org = computeMomentArmMuscleJoints(
                    mjc_model, muscle_name, joints, muscle_para_osim["mjc_coordinate_ranges"][i_jnts], 3
                )
                ...
                ma_aft = computeMomentArmMuscleJoints(
                    mjc_model, muscle_name, joints, muscle_para_osim["mjc_coordinate_ranges"][i_jnts], 3
                )
                ...
                if sum(abs((ma_aft.flatten() - ma_org.flatten()) / ma_org_nonzeros)) > 1e-3:
                    ...
                    ma_pnt = computeMomentArmMuscleJoints(
                        mjc_model, muscle_name, joints, muscle_para_osim["mjc_coordinate_ranges"][i_jnts], 3
                    )
...
def computeMomentArmMuscleJoints(mjc_model, muscle, joints, ang_ranges, evalN):
    ...
    for setAngleDofs in itertools.product(*ang_meshes):
        ...
        dependencyJnts, dependencyJntAngs, freeJnts = dependencyJointAng(mjc_model, joints_idx, setAngleDofs)
        mjc_data.qpos[dependencyJnts] = dependencyJntAngs
...
def updateWrapSites(mjc_model, wrap_type, wrap_id, pos_wrap, size_wrap, rot_wrap, side_ids, vec_optimized):
    ...
    if wrap_type == "TORUS":
        ...
        for isi, side_id in enumerate(side_ids):
            cylinderical_side = np.array([
                1.2 * vec_optimized[0],
                vec_optimized[5 + 2 * isi],
                vec_optimized[5 + 2 * isi + 1],
            ])
        ...
        for isi, side_id in enumerate(side_ids):
            spherical_side = np.array([
                1.2 * vec_optimized[0],
                vec_optimized[4 + 2 * isi],
                vec_optimized[4 + 2 * isi + 1],
            ])
        ...
        for isi, side_id in enumerate(side_ids):
            cylinderical_side = np.array([
                1.2 * size_wrap[0],
                vec_optimized[0 + 2 * isi],
                vec_optimized[0 + 2 * isi + 1],
            ])
            cartesian_side = cylindarical2cartesian(np.array([0, 0, 0]), cylinderical_side)
            rotation_side = quaternionRotaion(rot_wrap, cartesian_side) + pos_wrap
            mjc_model.site(side_id).pos = rotation_side
