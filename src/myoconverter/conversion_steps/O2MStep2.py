...
from myoconverter.optimization.utils.UtilsMujoco import (
    computeMomentArmMuscleJoints,
    getCoordinateRange_mjc,
    sortMuscleWrapSiteJoint,
)
...
class MomentArmOpt:
    ...
    def compMomentArmResults(self, mjc_model_path=None):
        ...
        pathlib.Path(self.save_path + "/moment_arms").mkdir(exist_ok=True, parents=True)
        ...
        # save the data based to the muscle parameter file
        with pathlib.Path(self.save_path + "/overall_comp_momentarms.pkl").open("wb") as rms_saving_file:
