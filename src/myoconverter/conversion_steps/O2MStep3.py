...
from myoconverter.optimization.utils.UtilsForceOpt import fmOptPSO_cust, getMuscleForceDiff
from myoconverter.optimization.utils.UtilsMujoco import getCoordinateRange_mjc, getMuscleForceLengthCurvesSim
...
class MuscleForceOpt:
    ...
    def __init__(self, mjc_model_path, osim_model_path, saving_path,
        ...
        try:
            pathlib.Path(saving_path).mkdir(exist_ok=True, parents=True)
        ...
        if osim_data_overwrite:
            ...
            for mus_name in self.muscle_list:
                ...
                with pathlib.Path(self.save_path + "/" + mus_name + ".pkl").open("wb") as muscle_saving:
    ...
    def optMuscleForce(self):
        ...
        for muscle in self.muscle_list:  # run through the muscle list
            ...
            # is reasonable
            fiber_len_range[0] = max(fiber_len_range[0], 0.01)
            ...
            if err_ind:
                ...
                with pathlib.Path(cvt3_model_path).open("w+") as xml_file:
            ...
            with pathlib.Path(self.save_path + "/" + muscle + ".pkl").open("wb") as muscle_file_saving:
    ...
    def compMuscleForceResults(self, mjc_model_path=None):
        ...
        pathlib.Path(self.save_path + "/muscle_forces").mkdir(exist_ok=True, parents=True)
        ...
        for i_mus, muscle in enumerate(muscle_list):
            ...
            if "opt_results" in muscle_para_opt.keys():
                ...
                # save the data based to the muscle parameter file
                with pathlib.Path(self.save_path + "/overall_comp_muscleforces.pkl").open("wb") as rms_saving_file:
