...
class BaseParser:
    ...
    def parse(self, xml, **kwargs):
        ...
        if xml.tag not in self.parsers:
            logger.critical(
                f"Parser for {self.component} type {xml.tag} has not been implemented. "
        ...
        if not isinstance(self.parsers[xml.tag], IParser):
            logger.error(
                f"Parser {self.parsers[xml.tag]} does not implement the {IParser} interface, something might break"
            )
