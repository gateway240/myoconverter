...
class CoordinateLimitForce(IParser):
    ...
    def parse(self, xml):
        ...
        if stiffness < 1e-4:
            ...
            force_coordinate_limits = (
                np.array([float(force["lower_limit"]), float(force["upper_limit"])]) * math.pi / 180
            )
            ...
            if target["limited"]:
                ...
                width = 0.001
                if "range" in target:
                    width_range = np.array([
                        force_coordinate_limits[0] - target["range"][0],
