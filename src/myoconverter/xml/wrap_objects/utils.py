...
def find_wrap_path(xml, body, tendon, center):
    ...
    if len(tendon_site_idxs) == 0:
        logger.warning(
            f"Could not find any sites for tendon {tendon.attrib['name']} that are located in body "
    ...
    # is last one)
        ...
        logger.warning(
            f"Could not place wrapping sites for tendon {tendon.attrib['name']} reliably. You might want to "
        ...
        if len(loc) > 0:
            ...
            if idx >= len(tendon_children) - 1 or (
                tendon_children[idx].tag == "site" and tendon_children[idx + 1].tag != "site"
            ):
