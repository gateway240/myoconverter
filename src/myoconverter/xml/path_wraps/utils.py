...
def add_wrapping_site(
    ...
    if default_sidesite is None:
        ...
        if rel_dist[point_idx] < 1:
            ...
            pos = wrap_object_pos + v * (WrapObject.sidesite_dist() + wrap_object_radius) / np.linalg.norm(v)
        else:
    ...
    return {
        "sidesite": sidesite,
        "wrap_object": wrap_object_name,
        "wrap_object_body": wrap_object_body,
        "predefined": True,
    }


def maybe_add_wrapping_site(
    idx,
    sites,
    muscle_name,
    wrap_object_name,
    ...
    if (
        M_WORLDBODY.find(f".//*body[@name='{s1.attrib['site']}']") is not None
