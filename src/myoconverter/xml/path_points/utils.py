...
def update_moving_path_point_location(coordinate_name, path_point):
    ...
    if mult_fn is not None:
        ...
        if multiplier != 1:
            logger.warning(
                f"Encountered a MultiplierFunction with scale != 1 in pathpoint {path_point.attrib['name']}."
...
def get_moving_path_point_dependency(path_point, coordinate_name, socket_name, cfg, output_dir):
    ...
    if mult_fn is not None:
        ...
        if multiplier != 1:
            logger.warning(
                f"Encountered a MultiplierFunction with scale != 1 in path point {path_point.attrib['name']}."
                f"This implementation has not been tested/validated, you better check it out yourself"
            )
    ...
    if function_type not in {"SimmSpline", "NaturalCubicSpline", "PiecewiseLinearFunction"}:
        logger.critical(
            f"Function type {function_type} not implemented (in {path_point.tag} {path_point.attrib['name']}). "
    ...
    if cfg.M_WORLDBODY.find(f".//joint[@name='{independent_coordinate}']") is None:
        logger.critical(
            f"Could not find a MuJoCo joint that corresponds to independent coordinate {independent_coordinate}"
        )
        raise RuntimeError
    ...
    if not cfg.TREAT_AS_NORMAL_PATH_POINT:
        fig = plot_figure(
            x_values, y_values, fit, independent_coordinate, path_point.attrib["name"], coordinate_name[0]
        )
