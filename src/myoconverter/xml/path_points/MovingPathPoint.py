...
class MovingPathPoint(IParser):
    ...
    def parse(self, xml, tendon, force_name, dependencies=None, **kwargs):
        ...
        if dependencies is None:
            joint_x, polycoef_x, range_x = get_moving_path_point_dependency(
                xml, "x_location", "socket_x_coordinate", cfg, output_dir
            )
            joint_y, polycoef_y, range_y = get_moving_path_point_dependency(
                xml, "y_location", "socket_y_coordinate", cfg, output_dir
            )
            joint_z, polycoef_z, range_z = get_moving_path_point_dependency(
                xml, "z_location", "socket_z_coordinate", cfg, output_dir
            )
        else:
        ...
        if cfg.TREAT_AS_NORMAL_PATH_POINT:
            ...
            self._pathpoint.parse(
                xml, tendon, force_name, pos=np.array([np.mean(range_x), np.mean(range_y), np.mean(range_z)])
            )
            return
        ...
        # each other.
        etree.SubElement(
            new_body, "geom", size="0.0005 0.0005 0.0005", rgba="0.0 0.0 1.0 0.0", contype="2", conaffinity="2"
        )
    ...
    def _add_joint(self, new_body, name, joint, polycoef, range, axis):
        ...
        :return: None
        """
        etree.SubElement(new_body, "joint", name=name, type="slide", axis=axis, range=vec2str(range))
        etree.SubElement(
            cfg.M_EQUALITY,
            "joint",
            joint1=name,
