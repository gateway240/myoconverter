...
class Body(IParser):
    ...
    def parse(self, xml, socket_parent_frame, socket_child_frame, current_body, root_body):
        ...
        inertial = etree.SubElement(next_body, "inertial", mass=xml.find("mass").text, pos=xml.find("mass_center").text)
        ...
        for mesh in xml.findall("attached_geometry/Mesh"):
            mesh_name, mesh_file = copy_mesh_file(
                mesh.find("mesh_file").text, cfg.GEOMETRY_FOLDER, cfg.OUTPUT_GEOMETRY_FOLDER
            )
            etree.SubElement(
                cfg.M_ASSET,
                "mesh",
            ...
            if cfg.ADD_GROUND_GEOM:
                etree.SubElement(cfg.M_CONTACT, "pair", geom1="ground-plane", geom2=mesh.attrib["name"])
