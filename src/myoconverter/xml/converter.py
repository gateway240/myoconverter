...
def _add_bodies_and_joints(parent_name, current_body, root_body=False):
    ...
    for s in socket_parent:
        ...
        # Parse body
        next_body = cfg.BODY_PARSER.parse(
            child_body,
...
def _copy_credits():
    ...
    if credits is not None:
        comment = etree.Comment(
            f" Credits from the original OpenSim model. Note! If the following has any license information, it applies to the original OpenSim model. Credits: {credits.text} "
        )
...
if __name__ == "__main__":
    argparser = argparse.ArgumentParser(
        description="Convert an OpenSim model into a MuJoCo model.Only Works with OpenSim v4 models."
    )
    """ ArgumentParser: Parse arguments when running module as a Python script. """
    argparser.add_argument("xml_file", type=str, help="Path to an OpenSim model XML file")
