...
def parse_coordinates(objects):
    ...
    for coordinate in objects:
        ...
        if default_value is not None:
            coordinates[coordinate.attrib["name"]].update({
                "user": float(default_value.text),
...
def plot_and_save_figure(x_values, y_values, fit, params, independent_coordinate, output_dir):
    ...
    pp.title(
        f"Approximation of dependent joint {params['name']} movement wrt independent joint {independent_coordinate}"
    )
