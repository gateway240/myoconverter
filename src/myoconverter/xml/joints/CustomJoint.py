    ...
    def _parse(self, xml, socket_parent_frame, socket_child_frame, pointer):
        ...
        for idx in order:
            ...
            if params["_transform_type"] == "rotation":
                ...
                logger.critical(
                    f"Unidentified transformation type {params['_transform_type']} for transformation {params['name']}"
                )
                raise TypeError
            ...
            # Handle a "SimmSpline" or "NaturalCubicSpline" transformation with a quartic approximation
                ...
                if t.find("SimmSpline") is not None:
                    ...
                    logger.critical(
                        "Could not find spline definition in the XML element; was expecting to find it under "
                ...
                coordinate_mapping = cfg.OPENSIM.xpath(
                    f"ConstraintSet//dependent_coordinate_name[text()='{coordinate.text}']"
                )
                if len(coordinate_mapping) > 1:
                    logger.critical(
                        f"Coordinate {coordinate.text} depends on two other coordinates! This type of relation has "
                ...
                if len(coordinate_mapping) == 1:
                    ...
                    if enforced is not None and not str2bool(enforced.text):
                        ...
                        if len(independent_coordinate.split(",")) > 1 or len(independent_coordinate.split(" ")) > 1:
                            logger.critical(
                                f"Coordinate {coordinate.text} depends on two other coordinates! This type of relation "
                        ...
                        if function is None or not np.array_equal(
                            str2vec(function.find("coefficients").text), np.array([1, 0])
                        ):
