...
def generate_pdf(
    mjc_vlt1_path: str, mjc_vlt2_path: str, mjc_vlt3_path: str, model_name: str, save_path: str
) -> Template:
    """
    ...
    pdf = Template(
        ...
        keywords="Musculoskeletal Model, MuJoCo, OpenSim, Muscle Kinematics & Kinetics, Conversion pipeline",
    )  # Landscape A4 (210 by 297 mm)

    def add_main_page(
        pdf: Template, model_name: str, mjc_vlt1_path: str, mjc_vlt2_path: str, mjc_vlt3_path: str
    ) -> Template:
        ...
        pdf["plant_text_title"] = (
            "The converted MJC model has been tested under the three categories below and with the accuracy of:"
        )

        pdf["list_title1"] = "- Step 1: XML Conversion Validation"
        pdf["list_content1"] = (
            "Check multi-body forward kinematics (using endpoints), approximation of custom/coupling joints"
            " & conditional/moving path points"
        )

        # load the end point data from the saved files
        if pathlib.Path(mjc_vlt1_path + "/end_points/end_point_error.pkl").is_file():
            with pathlib.Path(mjc_vlt1_path + "/end_points/end_point_error.pkl").open("rb+") as endpoint_error_file:
                endpoint_error = pickle.load(endpoint_error_file)

            pdf["error_content1"] = (
                "Mean error:"
                + str(round(endpoint_error["mean"] * 100, 4))
                + " cm"
                + "; std: "
                + str(round(endpoint_error["std"] * 100, 4))
                + " cm"
            )
        ...
        if pathlib.Path(mjc_vlt2_path + "/overall_comp_momentarms.pkl").is_file():
            with pathlib.Path(mjc_vlt2_path + "/overall_comp_momentarms.pkl").open("rb+") as momentarm_error_file:
                momentarm_error = pickle.load(momentarm_error_file)

            pdf["error_content2"] = (
                "Mean error:"
                + str(round(momentarm_error["cost_opt_mat_mean"] * 100, 4))
                + " cm"
                + "; std: "
                + str(round(momentarm_error["cost_opt_mat_std"] * 100, 4))
                + " cm"
            )

        pdf["list_title3"] = "- Step 3: Muscle Kinetic Validation"
        pdf["list_content3"] = (
            "Check muscle force-length relationship as indication of how similar of them in generating forces"
        )
        # load the muscle force error data from the saved files
        if pathlib.Path(mjc_vlt3_path + "/overall_comp_muscleforces.pkl").is_file():
            with pathlib.Path(mjc_vlt3_path + "/overall_comp_muscleforces.pkl").open("rb+") as muscleforce_error_file:
                muscleforce_error = pickle.load(muscleforce_error_file)

            pdf["error_content3"] = (
                "Mean error:"
                + str(round(muscleforce_error["rms_opt_mean"], 4))
                + " Fmax"
                + "; std: "
                + str(round(muscleforce_error["rms_opt_std"], 4))
                + " Fmax"
            )
    ...
    def add_step1_page(pdf: Template) -> Template:
        ...
        pdf["plant_text1"] = (
            "Randomly pose the model with 10 confgurations within the joint limits. "
    ...
    def add_step1_plots(pdf: Template, mjc_vlt1_path: str) -> Template:
        ...
        if pathlib.Path(mjc_vlt1_path + "/coordinate_coupler_constraints").is_dir():
            vlt1_plot_list = [
                os.path.split(f)[1][0:-4] for f in glob.glob(mjc_vlt1_path + "/coordinate_coupler_constraints/*.svg")
            ]
            # add all plots
            for vlt1_name in vlt1_plot_list:
                pdf.add_page()
        ...
        if pathlib.Path(mjc_vlt1_path + "/custom_joints").is_dir():
        ...
        if pathlib.Path(mjc_vlt1_path + "/moving_path_points").is_dir():
        ...
        # Scan the moving path folder to find all .svg files and add the overall comparison to the pdf report
        if pathlib.Path(mjc_vlt1_path + "/conditional_path_points").is_dir():
            vlt1_plot_list = [
                os.path.split(f)[1][0:-4] for f in glob.glob(mjc_vlt1_path + "/conditional_path_points/*.svg")
            ]
            # add all plots
            for vlt1_name in vlt1_plot_list:
                pdf.add_page()
    ...
    def add_step2_page(pdf: Template) -> Template:
        ...
        pdf["plant_text1"] = (
            "Moment arm of each muscle at each joint are compared between Osim and converted MJC model. "
    ...
    def add_step2_plots(pdf: Template, mjc_vlt2_path: str) -> Template:
        ...
        for vlt2_name in vlt2_plot_list:
            if "overall_comp" not in vlt2_name:
                pdf.add_page()
    ...
    def add_step3_page(pdf: Template) -> Template:
        ...
        pdf["plant_text1"] = (
            "Muscle force-length property are compared between Osim and Mjc models. "
    ...
    def add_step3_plots(pdf: Template, mjc_vlt3_path: str) -> Template:
        ...
        for vlt3_name in vlt3_plot_list:
            if "overall_comp" not in vlt3_name:
                pdf.add_page()
